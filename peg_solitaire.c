...
void testavailablemove (state_t *board);
...
int main(int argc, char *argv[]) {
	...
	if( ai_run ){ 
		...
		fprintf(fp_out, "\tExpanded nodes: %'d\n\tGenerated nodes: %'d\n", expanded_nodes, generated_nodes);
		fprintf(fp_out, "\tSolution Length: %d\n", solution_size);
		fprintf(fp_out, "\tNumber of Pegs Left: %d\n", num_pegs( &(solution[solution_size]) ) );
		fprintf(fp_out, "\tExpanded/seconds: %d\n", (int)(expanded_nodes/cpu_time_used) );
		fprintf(fp_out, "\tTime (seconds): %f\n", cpu_time_used );
		fclose(fp_out);
		...
		while (true) {
			...
			if (c=='t') {
				printf("Test Available Move\n");
				testavailablemove (board);
			}
...
void testavailablemove (state_t *board)
{
	int i;
	int j;
	for (i=0; i<SIZE; i++) // represent x axis 
	{
			for (j=0; j<SIZE; j++) //represent y axis
			{
				position_s curposition;
				curposition.x = i;
				curposition.y = j;
				if (can_apply(board, &curposition, left))
				{
					printf ("yang bisa jalan x = %i   y= %i  direction left\n", curposition.x, curposition.y);
				}
				if (can_apply(board, &curposition, right))
				{
					printf ("yang bisa jalan x = %i   y= %i  direction right\n", curposition.x, curposition.y);
				};
				if (can_apply(board, &curposition, up))
				{
					printf ("yang bisa jalan x = %i   y= %i  direction top\n", curposition.x, curposition.y);
				};
				if (can_apply(board, &curposition, down))
				{
					printf ("yang bisa jalan x = %i   y= %i  direction down\n", curposition.x, curposition.y);
				};
			}
		}
}