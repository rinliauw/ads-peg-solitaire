...
bool can_apply(state_t *board, position_s* selected_peg, move_t jump){
    ...
    switch (jump) {
        case up:          
            if(  selected_peg->y < 2) return false; // if  coordinate y is at 0,1 then cannot move
...
//bikin animasi supaya jalan ke atas
...
//check if masih ada yg bsb dijalanin
...
//buat print move up/down
char* action_cstr(move_t move){

	switch (move) {
		case up: // 2 di struct jadi up
			return "Up";
		break;
